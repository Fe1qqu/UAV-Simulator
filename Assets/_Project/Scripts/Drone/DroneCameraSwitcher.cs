...
public class DroneCameraSwitcher : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (cameras.Count == 0)
        {
            Debug.LogError("[DroneCameraSwitcher] No cameras assigned.");
            return;
        }
        ...
        {
            Debug.LogWarning(
                $"[DroneCameraSwitcher] defaultCameraIndex ({defaultCameraIndex}) " +
        ...
        for (int i = 0; i < cameras.Count; i++)
        {
            if (cameras[i] == null)
            {
                Debug.LogError($"[DroneCameraSwitcher] Camera at index {i} is null on object '{gameObject.name}'.");
                continue;
            }
    ...
    public void SwitchTo(int index)
    {
        ...
        {
            Debug.LogWarning($"[DroneCameraSwitcher] Invalid camera index: {index}.");
            return;
        }
